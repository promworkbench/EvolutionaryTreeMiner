...
import org.processmining.plugins.etm.fitness.metrics.EqualisedFitnessUsingDistance;
import org.processmining.plugins.etm.fitness.metrics.EqualisedFitnessUsingGeometry;
...
public class ETMFairParamFactory {
	...
	/**
	 * Standard weight for the equalized fairness based on distance quality dimension
	 */
	public static final double STD_EQUALIZED_DISTANCE_WEIGHT = 1;
	/**
	 * Standard weight for the equalized fairness based on angle quality dimension
	 */
	public static final double STD_EQUALIZED_GEOMETRIC_WEIGHT = 1;
	...
	public static ETMParam buildStandardParam(XLog eventlog, PluginContext context) {
		return buildParam(eventlog, context, STD_POPSIZE, STD_ELITE_COUNT, STD_RANDOMTREES_NR, STD_CROSSOVER_CHANCE,
				...
				STD_GENERALIZATION_WEIGHT, STD_SIMPLICITY_WEIGHT, STD_EQUALIZED_DISTANCE_WEIGHT, STD_EQUALIZED_GEOMETRIC_WEIGHT);
	}
	...
	public static ETMParam buildStandardParamConfigurable(UIPluginContext context, XLog[] logs) {
		return buildParamConfigurable(logs, context, STD_XEVENTCLASSIFIER, STD_POPSIZE, STD_ELITE_COUNT,
				...
				STD_EQUALIZED_DISTANCE_WEIGHT, STD_EQUALIZED_GEOMETRIC_WEIGHT, STD_CONFIGURATION_ALPHA, null);
	}
	...
	private static ETMParam buildParamConfigurable(XLog[] logs, UIPluginContext context,
			...
			double eqdWeight, double eqgWeight, double alpha, String loggingPath) {
	 ...
	 *            RECOMMENDATION: 10
	 * @param peWeight
	 *            double The weight of the Precision metric among all the
	 *            fitness metrics. RECOMMENDATION: 5
	 * @param geWeight
	 *            double The weight of the Generalization metric among all the
	 *            fitness metrics. RECOMMENDATION: 1
	 * @param sdWeight
	 *            double The weight of the Simplicity metric among all the
	 *            fitness metrics. RECOMMENDATION: 1
	 * @param eqdWeight
	 *            double The weight of the Equalized fairness metric based on distance among all the
	 *            fitness metrics. RECOMMENDATION: 1
	 * @param eqgWeight
	 *            double The weight of the Equalized fairness metric based on geometric interpretation among all the
	 *            fitness metrics. RECOMMENDATION: 1
	...
	public static ETMParam buildParam(XLog eventlog, final PluginContext context, int popSize, int eliteSize,
			...
			double geWeight, double sdWeight, double eqdWeight, double eqgWeight, NAryTree[] seed, double simWeight) {

		ETMParam param = new ETMParam();
		ETMFairParamFactory.<NAryTree>buildParamAbstract(param, eventlog, context, popSize, eliteSize, nrRandomTrees,
				crossOverChance, chanceOfRandomMutation, preventDuplicates, maxGen, targetFitness, frWeight,
				fitnessLimit, maxFTime, peWeight, geWeight, sdWeight, eqdWeight, eqgWeight, seed, simWeight, null);
		return param;
	}
	...
	private static <R> void buildParamAbstract(ETMParamAbstract<R> param, XLog eventlog, final PluginContext context,
			...
			double maxFTime, double peWeight, double geWeight, double sdWeight, double eqdWeight, double eqgWeight, NAryTree[] seed, double simWeight,
			String loggingPath) {
		...
		EqualisedFitnessUsingDistance eqd = new EqualisedFitnessUsingDistance(centralRegistry, canceller);
		EqualisedFitnessUsingGeometry eqg = new EqualisedFitnessUsingGeometry(centralRegistry, canceller);
	...
	public static ETMParamPareto buildETMParamPareto(XLog eventlog, PluginContext context, int popSize, int eliteSize,
		...
		ETMFairParamFactory.<ParetoFront>buildParamAbstract(param, eventlog, context, popSize, eliteSize, nrRandomTrees,
				crossOverChance, chanceOfRandomMutation, preventDuplicates, maxGen, targetFitness, frWeight, maxF,
				maxFTime, peWeight, geWeight, sdWeight, eqdWeight, eqgWeight, seed, simWeight, null);
